...
class ProcessL2:
    ...
    def spectralReflectance(node, sensor, timeObj, xSlice, F0, F0_unc, rhoScalar, rhoVec, waveSubset, xUNC):
        ...
        if ConfigFile.settings['bL1bCal'] >= 2 or ConfigFile.settings['SensorType'].lower() == 'seabird' :
            ...
            liUNC = xUNC[f'liUNC_{sensor}']
            ltUNC = xUNC[f'ltUNC_{sensor}']
            for i, wvl in enumerate(waveSubset):
                k = str(wvl)
                if (any([wvl == float(x) for x in esXSlice]) and any([wvl == float(x) for x in liXSlice]) and any(
                        [wvl == float(x) for x in ltXSlice])):
                    if sensor == 'HYPER':
        ...
        if ConfigFile.settings['bL2WeightSentinel3A']:
            ...
            if ConfigFile.settings['bL2WeightSentinel3A']:
                ...
                # if xUNC is not None:
                    ...
                    # xUNC['esUNC'] = xUNC['esUNC_Sentinel3A']
                    # xUNC['liUNC'] = xUNC['liUNC_Sentinel3A']
                    # xUNC['ltUNC'] = xUNC['ltUNC_Sentinel3A']
            ...
            if ConfigFile.settings['bL2WeightSentinel3B']:
                ...
                # if xUNC is not None:
                    ...
                    # xUNC['esUNC'] = xUNC['esUNC_Sentinel3B']
                    # xUNC['liUNC'] = xUNC['liUNC_Sentinel3B']
                    # xUNC['ltUNC'] = xUNC['ltUNC_Sentinel3B']
